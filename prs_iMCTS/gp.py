...
import random
import copy
from prs_iMCTS.src import ExpTree
...
class GPManager:
    ...
    def shrink_mutate(self, old_state: 'ExpTree', path: List[str]) -> List[str]:
        ...
        index = np.random.choice(valid_indices)
        if self.arity_dict[path[index]] == 1:
    ...
    def uniform_mutate(self, old_state: 'ExpTree', path: List[str], preferred_index: int = None) -> List[str]:

        if not path:
            return path

        if preferred_index is not None and 0 <= preferred_index < len(path):
    ...
    def insert_mutate(self, old_state: 'ExpTree', path: List[str]) -> List[str]:
        ...
        insert_ops = [op for op in self.ops if self.arity_dict[op] != 0]
        if not available_arity:
        ...
        if self.arity_dict[insert_op] == 1:
